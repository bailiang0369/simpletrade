"""H=10m 精确整除约数阵列 (1m, 2m, 5m, 10m) ResNet Pattern CNN 专项训练与评估

测试说明:
针对 H = 10m (预测未来 10 分钟涨跌，即 t+10 根 1min K 线价格)，
特征窗口严格使用 10 的精确整除公约数配比:
  - 1m  (10 根 1m K 线)
  - 2m  (5 根 2m K 线)
  - 5m  (2 根 5m K 线)
  - 10m (1 根 10m K 线)

验证 ResNet Pattern CNN 单模型在 H=10m 约数特征阵列下能否超越 70% 准确率！
"""
...
# ========== 1. 构建 H=10m 约数特征引擎 (1m, 2m, 5m, 10m) ==========
def build_h10_divisor_cnn_features(df_self, df_other=None):
...
# ========== 3. 运行 H=10m 专属约数 CNN 训练与评估 ==========
def run_h10_divisor_cnn(symbol='ETH'):
    horizon = 10
    print(f"\n" + "=" * 75, flush=True)
    print(f"  【H=10m 精确整除约数 (1m, 2m, 5m, 10m) ResNet Pattern CNN 评估】{symbol}", flush=True)
    print("=" * 75, flush=True)
    ...
    feats_cnn, feat_cols = build_h10_divisor_cnn_features(raw_self, raw_other)
    ...
    best_es_auc = 0.0
    best_path = os.path.join(config.MODEL_DIR, f"h10_div_cnn_{symbol}.pt")
    ...
    print(f"\n【H=10m 精确约数 (1m, 2m, 5m, 10m) {symbol} 评估结果】")
...
def main():
    print("=================================================================", flush=True)
    print("  H=10m 精确整除约数阵列 (1m, 2m, 5m, 10m) ResNet Pattern CNN 测试", flush=True)
    print("=================================================================", flush=True)

    run_h10_divisor_cnn('ETH')
    run_h10_divisor_cnn('BTC')
