"""Real-Time Three True Model Families Ensemble System (三大真正核心模型族实时聚合)

核心架构 (三大不同模型族，非全树形态):
  模型族 1: K线图形识别神经网络 (PatternResNet CNN) - 1D ResNet 深度学习族
  模型族 2: K线几何形态树模型 (Pattern GBDT) - 微观与高周期形态树族
  模型族 3: JOINT 多资产跨族池 (JOINT Pool20) - LGB/XGB/CAT 15个模型融合族

全部运行于 1min Tick 实时多周期特征之上 (5m, 15m, 30m, 60m, 120m, 240m 滑动窗口，包含当前 t 时刻未闭合 K 线)。
在 Meta-Val 段生成 Stacking 元学习器 / 秩归一化权重，并在全量 1min K 线测试段进行逐月与总体评估。
"""
...
# ========== 2. 模型族 1: ResNet 图形态神经网络 (1D ResNet CNN) ==========
...
class PatternResNet(nn.Module):
    ...
    def forward(self, x):
        # x shape: (B, lookback, in_channels) -> transpose -> (B, in_channels, lookback)
        x = x.transpose(1, 2)
        h = self.prep(x)
        h = self.res1(h)
        h = self.res2(h)
...
# ========== 4. 真正三大模型族 (CNN + GBDT + JOINT Pool) 训练与评估 ==========
def run_realtime_three_families(symbol='ETH', horizon=30):
    print(f"\n" + "=" * 75, flush=True)
    print(f"  【三大真正核心模型族实时聚合训练】{symbol} HORIZON={horizon}m", flush=True)
    print(f"  模型族 1: ResNet Pattern CNN (神经网络族)", flush=True)
    print(f"  模型族 2: Pattern GBDT (形态树族)", flush=True)
    print(f"  模型族 3: JOINT Pool20 (跨资产多族池)", flush=True)
    print("=" * 75, flush=True)
    ...
    # ---------- 1. 模型族 1: ResNet 图形态神经网络 (PatternResNet CNN) ----------
    print("\n[1/3] 训练【模型族 1】ResNet Pattern CNN (神经网络族)...", flush=True)
    ...
    best_es_auc = 0.0
    best_path = os.path.join(config.MODEL_DIR, f"realtime_cnn_family_{symbol}.pt")
    ...
    for ep in range(1, 4):
        ...
        with torch.no_grad():
            for bx, _ in es_loader:
                es_preds.append(cnn_model(bx.to(DEVICE)).cpu().numpy())
        es_auc = roc_auc_score(label[es_idx], np.concatenate(es_preds))
        print(f"  CNN Epoch {ep}/3 [{time.time()-t0:.1f}s] ES AUC: {es_auc:.4f}", flush=True)
    ...
    # ---------- 2. 模型族 2: Pattern GBDT (几何形态树模型) ----------
    print("\n[2/3] 训练【模型族 2】Pattern GBDT (形态树族)...", flush=True)
    ...
    # ---------- 3. 模型族 3: JOINT Pool20 跨资产融合族 ----------
    print("\n[3/3] 加载与训练【模型族 3】JOINT Pool20 (跨资产多族池)...", flush=True)
    ...
    # 1. 均值秩投票 (Rank Voting)
    r_voting_te = r_cnn_te * 0.2 + r_gbdt_te * 0.3 + r_joint_te * 0.5
    auc_voting = roc_auc_score(yte, r_voting_te)
    acc_voting, min_voting, bad_voting, tpd_voting, acc_m_voting = eval_r2_daily(r_voting_te, yte, ts_te)

    # 2. Logistic Stacking 元学习器
    ...
    acc_joint, min_joint, bad_joint, _, _ = eval_r2_daily(r_joint_te, yte, ts_te)

    m_mean_voting = np.mean(list(acc_m_voting.values()))
    m_mean_stacking = np.mean(list(acc_m_stacking.values()))

    print("\n" + "=" * 75, flush=True)
    print(f"  【三大真正核心模型族 ({symbol}) 实时聚合最终评估】", flush=True)
    print("=" * 75, flush=True)
    ...
    print(f"模型族 3: JOINT Pool20       -> AUC: {auc_joint:.4f} | Top1%准确率: {acc_joint*100:.2f}% | 最低月: {min_joint*100:.1f}% | 坏月: {bad_joint}个")
    print("-" * 75, flush=True)
    print(f"1. 秩投票 (Rank Voting)   -> Test AUC: {auc_voting:.4f} | 总体Top1%准确率: {acc_voting*100:.2f}% | 月均准确率: {m_mean_voting*100:.2f}% | 最低月: {min_voting*100:.1f}% | 坏月: {bad_voting}个")
    print(f"2. Stacking 元学习器       -> Test AUC: {auc_stacking:.4f} | 总体Top1%准确率: {acc_stacking*100:.2f}% | 月均准确率: {m_mean_stacking*100:.2f}% | 最低月: {min_stacking*100:.1f}% | 坏月: {bad_stacking}个")
    print(f"Stacking 解出元权重: CNN={meta_learner.coef_[0][0]:.3f}, PatternGBDT={meta_learner.coef_[0][1]:.3f}, JOINT={meta_learner.coef_[0][2]:.3f}")
    print(f"\n  {symbol} Stacking 集成 12 个月逐月明细:\n  {acc_m_stacking}\n")

def main():
    print("=================================================================", flush=True)
    print("  SimpleTrade 三大真正核心模型族 (CNN + Pattern GBDT + JOINT) 实时聚合", flush=True)
    print("=================================================================", flush=True)

    run_realtime_three_families('ETH', horizon=30)
    run_realtime_three_families('BTC', horizon=30)
