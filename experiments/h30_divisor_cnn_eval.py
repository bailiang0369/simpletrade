"""H=30m 精确整除约数阵列 (2m, 6m, 10m, 30m) ResNet Pattern CNN 专项训练与评估

测试说明:
针对 H = 30m (预测未来 30 分钟涨跌) 目标，特征视窗严格使用 30 的公约数进行等倍对齐:
  - 1m  (30 根 1m K 线)
  - 2m  (15 根 2m K 线)
  - 6m  (5 根 6m K 线)
  - 10m (3 根 10m K 线)
...
验证 ResNet Pattern CNN 单模型在 H=30m 下的表现能否大幅冲高至 70%+！
"""
...
# ========== 1. 构建 H=30m 精确整除约数特征引擎 (2m, 6m, 10m, 30m) ==========
...
# ========== 3. 运行 H=30m 专属约数 CNN 训练与评估 ==========
def run_h30_divisor_cnn(symbol='ETH'):
    horizon = 30
    print(f"\n" + "=" * 75, flush=True)
    print(f"  【H=30m 精确整除约数 (2m, 6m, 10m, 30m) ResNet Pattern CNN 评估】{symbol}", flush=True)
    print("=" * 75, flush=True)
    ...
    feats_cnn, feat_cols = build_h30_divisor_cnn_features(raw_self, raw_other)
    ...
    best_es_auc = 0.0
    best_path = os.path.join(config.MODEL_DIR, f"h30_div_cnn_{symbol}.pt")
    ...
    print(f"\n【H=30m 精确约数 (2m, 6m, 10m, 30m) {symbol} 评估结果】")
...
def main():
    print("=================================================================", flush=True)
    print("  H=30m 精确整除约数阵列 (2m, 6m, 10m, 30m) ResNet Pattern CNN 测试", flush=True)
    print("=================================================================", flush=True)

    run_h30_divisor_cnn('ETH')
    run_h30_divisor_cnn('BTC')
