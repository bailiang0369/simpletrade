...
模型族 1: ResNet Pattern CNN (神经网络族)
模型族 2: Pattern GBDT (形态树族)
模型族 3: JOINT Pool20 (跨资产多族池)
"""
...
def run_h15_divisor_stacking(symbol='ETH'):
    ...
    if symbol == 'ETH':
        feats, feat_names = build_h15_divisor_features(raw_e, raw_b)
        ts = raw_e['ts'].values.astype(np.int64)
        close = raw_e['close'].values
    else:
        feats, feat_names = build_h15_divisor_features(raw_b, raw_e)
