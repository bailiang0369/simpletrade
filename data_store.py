...
class AssetContext:
    def __init__(self, symbol, horizon=None):
        ...
        self.tb = pq.read_table(raw_p, columns=["buy_vol"])["buy_vol"].to_numpy().astype(np.float32)
        self.vol = pq.read_table(raw_p, columns=["sell_vol"])["sell_vol"].to_numpy().astype(np.float32)
        ...
        # 切分掩码: 直接比较 epoch 秒, 半开区间 [a, b) 保证相邻切分互斥(不重复计入边界分钟)
        ts_s = self.ds_ts
        self.split_rows = {}
    ...
    def times(self, name):
        # 返回 numpy datetime64[s] 数组(轻量, 不再依赖 pandas)
        return self.ds_ts[self.split_rows[name]].astype("datetime64[s]")
    ...
    def raw_channels(self):
        ...
        tb = self.tb              # 主动买量
        ts = self.vol             # 主动卖量 (raw parquet 中已为 sell_vol)
        rng = h - l
