...
from download_data import download_all, download_current_month_daily
...
def run_report(symbols=None):
    ...
    try:
        ...
        tt = pd.to_datetime(d["tt"])
        k = int(len(Pm) * config.COVERAGE)
        sel = np.argsort(-conf)[:k]
        trades = pd.DataFrame({
            "time": tt[sel], "symbol": symbols[0], "prob": Pm[sel],
            "direction": pred[sel], "label": d["yt"][sel], "ret15m": d["rt"][sel],
        })
        trades.to_csv(os.path.join(config.RESULT_DIR, "trades_meta_btc.csv"), index=False)
