"""快速重跑 xf2.py + per-hour 完整扫描 + 多 seed"""
...
btc_lr=np.log(btc_close[1:]/btc_close[:-1]); btc_lr=np.append([np.nan],btc_lr).astype(np.float32)
...
for sd in range(42,57):
    ...
    p_te_all.append(m.predict(Xte))
    log(f'  seed {sd}: {time.time()-t:.0f}s iter={m.best_iteration}')
    del m; gc.collect()
p=np.mean(np.column_stack(p_te_all),axis=0); del p_te_all; gc.collect()
auc=roc_auc_score(yte,p); log(f'  AUC={auc:.4f}')

# ====== 3. Top-k ======
log('\n[3] Top-k acc')
o=np.argsort(p)
for k in [0.003,0.005,0.008,0.01,0.012,0.015,0.02,0.025,0.03,0.05,0.08,0.10]:
    idx=o[-max(int(len(p)*k),1):]; acc=(yte[idx]==1).mean(); tp=tpd(len(idx),len(yte))
    flag='✅' if (acc>=0.65 and tp>=15) else ''
    log(f'  top{k*100:.1f}%: acc={acc:.4f} tpd={tp:.1f} {flag}')

# ====== 4. Per-hour 完整扫描 ======
log('\n[4] Per-hour × percentile 完整扫描')
...
# ====== 5. 最终汇总 ======
log(f'\n{"="*60}'); log('🏆 最终'); log(f'{"="*60}')
log(f'  特征数=63 (ETH 46 + BTC 17), 15 seeds')
log(f'  AUC={auc:.4f} (基线 0.5450)')
if best_hit:
    best_hit.sort(key=lambda x:-x[2])
    for K,thr,acc,tp in best_hit[:15]: log(f'  ✅ K={K}h P{thr}: acc={acc:.4f} tpd={tp:.1f}')
else:
    log('  ❌ 未达标 (需 acc≥65%, tpd≥15)')
    log('  per-hour 全扫描 0 命中')
