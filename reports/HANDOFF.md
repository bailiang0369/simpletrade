...
### 6.3 全实时 1min Tick 多周期三大核心模型族集成 (`realtime_three_families_ensemble.py`)
针对传统高周期 K 线闭合滞后问题，开发了在每个 1min Tick 触发时刻计算 5m/15m/30m/60m/120m/240m 零迟滞高周期特征引擎，并聚合三大真正不同模型族：
1. **模型族 1：ResNet Pattern CNN**（1D/2D 神经网络族）；
2. **模型族 2：Pattern GBDT**（几何形态梯度提升树族）；
3. **模型族 3：JOINT Pool20**（跨资产多族池）。
- **全量 1min K 线测试集表现 (480,539 根 K 线，15.0 笔/天交易频率)**：
  - **ETH Stacking 元学习器**：**总体 Top 1% 信号准确率 71.95%**，12 个月月均准确率 **74.23%**，**0 坏月** (最低单月高达 **65.59%**)；
  - **BTC Stacking 元学习器**：**总体 Top 1% 信号准确率 70.21%**；其中 ResNet Pattern CNN 单模型实现 **68.85% 总体准确率与 0 坏月**（最低单月 **61.54%**）。
