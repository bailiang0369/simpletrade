...
- **核心结论**：
  ...
  3. **H=15m 约数精准配比 (`strict_divisor_h15_stacking.py`)**：只使用 15 的整除约数 (`1m, 3m, 5m, 15m`) 时，**ResNet Pattern CNN 单模型在 ETH H=15m 上暴冲至 71.53% 总体准确率且最低单月高达 66.0% (0 坏月)**！证实了多基础 K 线视窗是神经网络识别微观形态的强力武器。
