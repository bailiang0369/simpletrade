"""ENS_FULLBTC2: 87 feats (62 ETH + 25 BTC), LGB+CB."""
...
# ===== FEATURE IMPORTANCE =====
print('\n[4] Top feats (LGB H=30)...', flush=True)
D2 = np.load('/workspace/models/eth_full.npz', allow_pickle=True)
X_for_imp = D2['X'].astype(np.float32)
tr_idx_imp = np.where(ts_mask('2020-01-01','2024-06-30'))[0]
es_idx_imp = np.where(ts_mask('2024-06-30','2024-09-30'))[0]
m_imp = lgb.train({'objective':'binary','verbose':-1,'n_jobs':3,'seed':42},
                    lgb.Dataset(X_for_imp[tr_idx_imp], y_all[30][tr_idx_imp]),
                    num_boost_round=500,
                    valid_sets=[lgb.Dataset(X_for_imp[es_idx_imp], y_all[30][es_idx_imp])],
                    callbacks=[lgb.early_stopping(50), lgb.log_evaluation(0)])
imps = m_imp.feature_importance(importance_type='gain')
top = np.argsort(-imps)[:25]
print('  Top 25:')
for i in top:
    tag = 'BTC ' if any(k in feat_names[i] for k in ['btc','corr','spread']) else '    '
    print(f'    {tag} {i:>2d} {feat_names[i]:>18s}  gain={imps[i]:>10.0f}')
del m_imp, D2, X_for_imp; gc.collect()
