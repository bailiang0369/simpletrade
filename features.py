...
严格遵守用户约束:
- 仅使用价格衍生特征 (O/H/L/C 及其滚动统计) 与主动买卖成交量 (taker_buy_base 及其滚动统计)
...
def build_features(df: pl.DataFrame) -> pl.DataFrame:
    """df: polars DataFrame, 列含 ts(int64秒)/open/high/low/close/volume/taker_buy_base。
    ...
    # 组内累计序号(自1起); 乘 green/(1-green) 清零反向K线
    run_idx = tmp.select(pl.col("_g").cum_count().over("_grp")).to_series().cast(pl.Float64)
    dn_run_idx = tmp.select((1 - pl.col("_g")).cum_count().over("_grp")).to_series().cast(pl.Float64)
    out = out.with_columns([
        (green.cast(pl.Float64) * run_idx / 20).clip(0, 1).alias("streak_up"),
        ((1 - green.cast(pl.Float64)) * dn_run_idx / 20).clip(0, 1).alias("streak_dn"),
    ])
