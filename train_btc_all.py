"""训练 BTC h10 + h60 (LGB+CB 10-seed each)"""
...
def run_lgb(sym,h):
    ...
    if os.path.exists(f"{OUT}/lgb_{tag}_te.npy"):
        print(f"  [LGB {tag}] 已存, 跳过"); return
    Xtr,ytr=load(sym,h,"train"); Xes,yes=load(sym,h,"early_stop")
    print(f"[LGB {tag}] train={Xtr.shape} {now()}",flush=True)
    ...
    mv=rank_ens(np.array(Pmv)); te=rank_ens(np.array(Pte))
    np.save(f"{OUT}/lgb_{tag}_mv.npy",mv); np.save(f"{OUT}/lgb_{tag}_te.npy",te)
    ymv=np.load(f"{NPY}/{sym}_h{h}_meta_val_y.npy"); yte=np.load(f"{NPY}/{sym}_h{h}_test_y.npy")
    print(f"  [LGB {tag}] mv={roc_auc_score(ymv,mv):.4f} te={roc_auc_score(yte,te):.4f} {now()}",flush=True)

def run_cb(sym,h):
    tag = f"{sym}_h{h}"
    if os.path.exists(f"{OUT}/cb_{tag}_te.npy"):
        print(f"  [CB {tag}] 已存, 跳过"); return
    Xtr,ytr=load(sym,h,"train"); Xes,yes=load(sym,h,"early_stop")
    print(f"[CB {tag}] train={Xtr.shape} {now()}",flush=True)
    ...
    del Xtr,ytr,Xes,yes; gc.collect()
    mv=rank_ens(np.array(Pmv)); te=rank_ens(np.array(Pte))
    np.save(f"{OUT}/cb_{tag}_mv.npy",mv); np.save(f"{OUT}/cb_{tag}_te.npy",te)
    ymv=np.load(f"{NPY}/{sym}_h{h}_meta_val_y.npy"); yte=np.load(f"{NPY}/{sym}_h{h}_test_y.npy")
    print(f"  [CB {tag}] mv={roc_auc_score(ymv,mv):.4f} te={roc_auc_score(yte,te):.4f} {now()}",flush=True)
