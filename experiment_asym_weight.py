#!/usr/bin/env python3
"""实验: 非对称加权 - 通过提高负样本权重减少假阳性。

基于错误分析发现 72% 的错误为假阳性 (模型预测 UP 但实际 DOWN)。
通过给负样本 (y=0, 实际下跌) 赋予更高权重, 使模型更保守, 减少假阳性。

与 experiment_enhanced.py 的关系:
- 完全相同的特征集 (FEATURES + EXTRA_FEATURE_NAMES)
- 完全相同的 compute_extra_raw / get_extra_for_mask
- 完全相同的 5-seed bagging (42, 49, 56, 63, 70)
- 完全相同的 scale_pos_weight=1.0
- 完全相同的坏时段加权 (hour 17-20, 0-5 权重 2x)
- 完全相同的 Platt 后校准
- 完全相同的排名平均组合

唯一变化: 在加权采样中, 对负样本 (y=0) 额外乘以 neg_multiplier,
  正样本 (y=1) 权重不变。

训练流程:
1. Baseline (neg_multiplier=1.0, 等价于 enhanced)
2. 不同 multiplier: 1.5, 2.0, 3.0, 5.0
3. 对比各设置下的准确率、收益、假阳性率
"""

import os
import sys
import gc
import time
import textwrap

import lightgbm as lgb
import numpy as np

sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
import config
from data_store import AssetContext
from evaluate import evaluate_topk


# ============================================================
# 特征定义 (与 experiment_enhanced.py 完全一致)
# ============================================================
FEATURES = [
    "lr_5", "lr_15", "lr_30", "lr_120", "lr_240", "mom_60",
    "z_10", "z_30", "z_60", "z_120",
    "rvol_30", "rvol_60", "rvol_ratio_60_5", "rvol_z_60", "rvol_dir",
    "pos_30", "pos_60", "pos_120", "pos_240",
    "dd_240", "ru_240",
    "hh_dd_60", "ll_ru_60", "body_pos_60",
    "body_ratio", "up_wick", "lo_wick", "ngreen_10", "gap", "max_range_30",
    "tbr_z_30", "cvd_30", "cvd_60",
    "buyvol_strength_30", "tb_act_60", "ts_act_60", "tb_acc_30",
    "cvd_dir_30", "tbr_hi_60", "lr_skew_60", "up_body_ratio_30", "mom_align_30_240",
    "hour_sin", "hour_cos", "dow_sin", "dow_cos", "is_us", "is_eu", "ret_day",
    "pos_tbr_interact", "vol_mom_interact", "pos_cvd_interact",
    "di_spread", "di_uptrend", "mom_vol_confirm", "z_divergence", "cvd_accel", "vol_cvd_interact", "di_plus",
]

EXTRA_FEATURE_NAMES = [
    "hour_sin_is_us",
    "hour_cos_is_eu",
    "hour_sin_rvol_60",
    "consec_up",
    "consec_dn",
    "session_minutes",
    "hour_sin_hour_cos",
]

BAGGED_SEEDS = [42, 49, 56, 63, 70]


# ============================================================
# 自定义评价函数 (与 experiment_enhanced.py 完全一致)
# ============================================================
def topk_acc_eval(preds, train_data):
    labels = train_data.get_label()
    probs = 1.0 / (1.0 + np.exp(-preds))
    n = len(probs)
    k = max(1, int(n * 0.01))
    conf = np.abs(probs - 0.5) * 2
    sel = np.argpartition(-conf, k)[:k]
    acc = (labels[sel] > 0.5).mean()
    return 'top1_acc', acc, True


# ============================================================
# 新增特征计算 (与 experiment_enhanced.py 完全一致)
# ============================================================
def compute_extra_raw(ctx):
    """为全部原始数据计算新增特征, 返回 dict of float32 arrays。"""
    t0 = time.time()
    close = ctx.c
    raw_ts = ctx.raw_ts
    n = len(close)

    # ---- 1. 分钟对数收益 ----
    lr_1 = np.zeros(n, dtype=np.float64)
    lr_1[1:] = np.log(close[1:] / close[:-1])

    # ---- 2. rvol_60 (raw级别, rolling std, 60窗, ddof=1) ----
    rvol_60 = np.zeros(n, dtype=np.float32)
    w = 60
    if n >= w:
        cs = np.cumsum(lr_1)
        cs2 = np.cumsum(lr_1 ** 2)
        roll_sum = cs[w:] - cs[:-w]
        roll_sum2 = cs2[w:] - cs2[:-w]
        roll_mean = roll_sum / w
        roll_var = roll_sum2 / w - roll_mean ** 2
        roll_var = np.maximum(roll_var, 0)
        roll_std = np.sqrt(roll_var * w / (w - 1))
        rvol_60[w:] = (roll_std * 100).astype(np.float32)

    # ---- 3. 时间特征 ----
    hour = (raw_ts % 86400) // 3600
    minute_of_day = (raw_ts % 86400) // 60
    hour_sin = np.sin(hour * 2 * np.pi / 24).astype(np.float32)
    hour_cos = np.cos(hour * 2 * np.pi / 24).astype(np.float32)
    is_us = ((hour >= 13) & (hour < 21)).astype(np.float32)
    is_eu = ((hour >= 8) & (hour < 13)).astype(np.float32)

    # ---- 4. 连续同向K线计数 ----
    consec_up = np.zeros(n, dtype=np.int32)
    consec_dn = np.zeros(n, dtype=np.int32)
    for i in range(1, n):
        if close[i] > close[i - 1]:
            consec_up[i] = consec_up[i - 1] + 1
        else:
            consec_dn[i] = consec_dn[i - 1] + 1
    consec_up_f = np.clip(consec_up.astype(np.float32) / 50.0, 0, 1)
    consec_dn_f = np.clip(consec_dn.astype(np.float32) / 50.0, 0, 1)

    # ---- 5. Session minutes ----
    session_minutes = np.zeros(n, dtype=np.float32)
    for i in range(n):
        h = hour[i]
        m = minute_of_day[i]
        if h < 8:
            session_minutes[i] = m
        elif h < 13:
            session_minutes[i] = m - 8 * 60
        elif h < 21:
            session_minutes[i] = m - 13 * 60
        else:
            session_minutes[i] = m - 21 * 60
    session_minutes = session_minutes / 480.0

    extra = {
        "hour_sin_is_us": (hour_sin * is_us).astype(np.float32),
        "hour_cos_is_eu": (hour_cos * is_eu).astype(np.float32),
        "hour_sin_rvol_60": (hour_sin * rvol_60).astype(np.float32),
        "consec_up": consec_up_f.astype(np.float32),
        "consec_dn": consec_dn_f.astype(np.float32),
        "session_minutes": session_minutes.astype(np.float32),
        "hour_sin_hour_cos": (hour_sin * hour_cos).astype(np.float32),
    }

    print(f"  [extra] 计算了 {len(extra)} 个新增特征, "
          f"耗时 {time.time() - t0:.1f}s", flush=True)
    return extra


def get_extra_for_mask(extra_raw, ctx, mask):
    """从 extra_raw 中提取 mask 对应的行, 按 EXTRA_FEATURE_NAMES 顺序返回矩阵。"""
    ri = ctx.ds_to_raw[mask].astype(int)
    cols = []
    for name in EXTRA_FEATURE_NAMES:
        cols.append(extra_raw[name][ri])
    return np.column_stack(cols)


# ============================================================
# 训练 (与 experiment_enhanced.py 一致, 增加 neg_multiplier 参数)
# ============================================================
def train_asym_weight_gbdt(ctx, extra_raw, neg_multiplier=1.0, max_train=2_600_000):
    """训练非对称加权 GBDT: 5-seed bagging + 新特征 + 负样本加权。

    Parameters
    ----------
    ctx : AssetContext
    extra_raw : dict
        compute_extra_raw() 返回的新增特征.
    neg_multiplier : float
        负样本 (y=0) 的权重乘数. 正样本 (y=1) 权重不变.
        neg_multiplier=1.0 等价于 experiment_enhanced.
    max_train : int
        训练集最大采样数.

    Returns
    -------
    models : list of lgb.Booster
    calibrator : LogisticRegression or None
    """
    t0 = time.time()
    feats = list(FEATURES)
    trm = ctx.split_rows["train"]
    esm = ctx.split_rows["early_stop"]
    tr_idx_all = np.where(trm)[0]

    # ---- 早停集 ----
    Xes_base = ctx.X_subset(feats, esm)
    Xes_extra = get_extra_for_mask(extra_raw, ctx, esm)
    Xes = np.column_stack([Xes_base, Xes_extra])
    yes = ctx.label[esm].astype(np.float64)
    del Xes_base, Xes_extra
    gc.collect()

    train_retf = ctx.retf("train")

    models = []
    for seed in BAGGED_SEEDS:
        rng = np.random.default_rng(seed)
        tr_idx = tr_idx_all.copy()
        if len(tr_idx) > max_train:
            keep = rng.choice(len(tr_idx), max_train, replace=False)
            tr_idx = tr_idx[keep]

        train_mask = np.zeros_like(trm, dtype=bool)
        train_mask[tr_idx] = True

        Xtr_base = ctx.X_subset(feats, train_mask)
        Xtr_extra = get_extra_for_mask(extra_raw, ctx, train_mask)
        Xtr = np.column_stack([Xtr_base, Xtr_extra])
        del Xtr_base, Xtr_extra
        gc.collect()

        ytr = ctx.label[train_mask].astype(np.float64)

        # ---- 加权采样 ----
        if len(tr_idx) < len(tr_idx_all):
            keep_local = np.where(train_mask[tr_idx_all])[0]
            raw_w = np.abs(train_retf[keep_local]).astype(np.float64)
        else:
            raw_w = np.abs(train_retf).astype(np.float64)

        w = np.clip(raw_w * 50, 0.5, 5.0)

        # ---- 坏时段加权 ----
        train_ds_ts = ctx.ds_ts[train_mask]
        train_hour = (train_ds_ts % 86400) // 3600
        bad_hour = ((train_hour >= 17) & (train_hour <= 20)) | (train_hour <= 5)
        if bad_hour.any():
            w[bad_hour] *= 2.0
            print(f"  [weight] 坏时段样本 {bad_hour.sum()}/{len(bad_hour)} "
                  f"({bad_hour.mean() * 100:.1f}%) 权重 2x", flush=True)

        # ================================================================
        # 非对称加权: 负样本 (y=0) 权重乘以 neg_multiplier
        # ================================================================
        neg_mask = (ytr == 0)
        if neg_multiplier != 1.0 and neg_mask.any():
            w[neg_mask] *= neg_multiplier
            print(f"  [asym_weight] neg_multiplier={neg_multiplier:.2f}: "
                  f"负样本 {neg_mask.sum()}/{len(neg_mask)} "
                  f"({neg_mask.mean() * 100:.1f}%) 权重 x{neg_multiplier:.2f}", flush=True)

        # ---- LightGBM 参数 ----
        params = dict(
            objective="binary",
            metric="auc",
            learning_rate=0.02,
            num_leaves=127,
            max_depth=-1,
            feature_fraction=0.8,
            bagging_fraction=0.8,
            bagging_freq=2,
            min_data_in_leaf=100,
            lambda_l1=0.05,
            lambda_l2=1.0,
            scale_pos_weight=1.0,
            num_threads=config.N_JOBS,
            verbosity=-1,
            seed=seed,
            min_data=1,
        )

        dtr = lgb.Dataset(Xtr, ytr, weight=w)
        des = lgb.Dataset(Xes, yes, reference=dtr)
        m = lgb.train(
            params, dtr,
            num_boost_round=5000,
            valid_sets=[des],
            valid_names=['early_stop'],
            feval=topk_acc_eval,
            callbacks=[
                lgb.early_stopping(200, verbose=False, min_delta=1e-5),
                lgb.log_evaluation(0),
            ],
        )
        models.append(m)

        auc_val = m.best_score['early_stop'].get('auc', -1)
        topk_val = m.best_score['early_stop'].get('top1_acc', -1)
        print(f"  [asym_weight] seed{seed} best_iter={m.best_iteration} "
              f"auc={auc_val:.4f} top1_acc={topk_val:.4f} "
              f"({time.time() - t0:.0f}s)", flush=True)

        del Xtr, dtr, ytr, w
        gc.collect()

    # ---- Platt 后校准 ----
    calibrator = None
    if "meta_val" in ctx.split_rows:
        from sklearn.linear_model import LogisticRegression
        cal_mask = ctx.split_rows["meta_val"]
        cal_p = _predict_raw(ctx, models, extra_raw, cal_mask)
        cal_y = ctx.label[cal_mask]
        fin = np.isfinite(cal_p)
        if fin.sum() > 1000:
            calibrator = LogisticRegression(C=1.0, max_iter=500, random_state=42)
            logit_p = np.clip(cal_p[fin], 1e-7, 1 - 1e-7)
            logit_p = np.log(logit_p / (1 - logit_p)).reshape(-1, 1)
            calibrator.fit(logit_p, cal_y[fin])
            print(f"  [asym_weight] Platt校准: 在meta_val上拟合 "
                  f"({fin.sum()}样本, coef={calibrator.coef_[0][0]:.4f})", flush=True)
        else:
            print(f"  [asym_weight] 跳过Platt校准: 有效样本 {fin.sum()} 不足", flush=True)

    nfeat = len(feats) + len(EXTRA_FEATURE_NAMES)
    print(f"  [asym_weight] neg_mult={neg_multiplier:.2f} "
          f"{len(models)}-seed bagging fit done in "
          f"{time.time() - t0:.0f}s (nfeat={nfeat})", flush=True)
    return models, calibrator


def _predict_raw(ctx, models, extra_raw, mask):
    """原始预测 (排名平均, 未校准), 返回 float64 概率。"""
    X_base = ctx.X_subset(FEATURES, mask)
    X_extra = get_extra_for_mask(extra_raw, ctx, mask)
    X = np.column_stack([X_base, X_extra])
    del X_base, X_extra
    gc.collect()

    n = len(X)
    R = np.zeros((len(models), n), dtype=np.float64)
    for i, m in enumerate(models):
        p = m.predict(X, num_iteration=m.best_iteration)
        R[i] = np.argsort(np.argsort(p)).astype(np.float64) / (n - 1)
    del X
    gc.collect()
    return np.asarray(R.mean(axis=0), dtype=np.float64)


def predict_asym_weight(ctx, models, calibrator, extra_raw, split):
    """预测: 排名平均 + 可选 Platt 校准。返回 [0,1] float32。"""
    mask = ctx.split_rows[split]
    p_raw = _predict_raw(ctx, models, extra_raw, mask)
    if calibrator is not None:
        logit_p = np.clip(p_raw, 1e-7, 1 - 1e-7)
        logit_p = np.log(logit_p / (1 - logit_p)).reshape(-1, 1)
        p = calibrator.predict_proba(logit_p)[:, 1].astype(np.float64)
    else:
        p = p_raw
    return np.asarray(p, dtype=np.float32)


# ============================================================
# 假阳性率评估
# ============================================================
def eval_false_positive_rate(p, y):
    """计算假阳性率: 模型预测 UP 但实际 DOWN 的比例。

    Parameters
    ----------
    p : array-like, 预测概率
    y : array-like, 真实标签 (0=DOWN, 1=UP)

    Returns
    -------
    dict with keys: fp_rate, fp_count, total_pred_up, fn_rate, fn_count, total_pred_dn
    """
    p = np.asarray(p, dtype=np.float64)
    y = np.asarray(y, dtype=np.int8)
    pred = (p >= 0.5).astype(np.int8)

    pred_up = (pred == 1)
    pred_dn = (pred == 0)

    fp = (pred_up) & (y == 0)   # 预测涨但实际跌
    fn = (pred_dn) & (y == 1)   # 预测跌但实际涨

    total_up = pred_up.sum()
    total_dn = pred_dn.sum()

    return {
        "fp_rate": float(fp.sum() / total_up) if total_up > 0 else 0.0,
        "fp_count": int(fp.sum()),
        "total_pred_up": int(total_up),
        "fn_rate": float(fn.sum() / total_dn) if total_dn > 0 else 0.0,
        "fn_count": int(fn.sum()),
        "total_pred_dn": int(total_dn),
        "accuracy": float((pred == y).mean()),
    }


# ============================================================
# 主函数
# ============================================================
def main():
    symbol = "BTC"
    neg_multipliers = [1.0, 1.5, 2.0, 3.0, 5.0]

    print(f"\n{'=' * 65}")
    print(f"  非对称加权实验: {symbol} H30")
    print(f"  - 与 experiment_enhanced.py 完全相同的特征集")
    print(f"  - 在加权采样基础上, 对负样本 (y=0) 额外乘以 neg_multiplier")
    print(f"  - 正样本 (y=1) 权重不变")
    print(f"  - 测试 multiplier: {neg_multipliers}")
    print(f"  - {len(BAGGED_SEEDS)}-seed bagging + Platt 校准 + 排名平均")
    print(f"{'=' * 65}\n")

    t_start = time.time()

    # ================================================================
    # 1. 加载数据 (仅需一次, 所有实验共享)
    # ================================================================
    print("[1/4] 加载数据...")
    ctx = AssetContext(symbol, horizon=30)
    print(f"  [ctx] 数据集行数: {len(ctx.ds_ts)}, "
          f"特征数: {len(ctx.feat_names)}", flush=True)

    # ================================================================
    # 2. 计算新增特征 (仅需一次)
    # ================================================================
    print("[2/4] 计算新增特征...")
    extra_raw = compute_extra_raw(ctx)

    # 预获取测试集标签和收益 (所有实验共享)
    y_te = ctx.y("test")
    retf_te = ctx.retf("test")
    ts_te = ctx.times("test")

    # ================================================================
    # 3. 遍历所有 neg_multiplier 训练并评估
    # ================================================================
    print(f"[3/4] 训练 {len(neg_multipliers)} 个加权配置...\n")

    all_results = {}       # neg_mult -> result dict
    all_fp_rates = {}      # neg_mult -> fp_rate dict
    all_models = {}        # neg_mult -> (models, calibrator)

    for nm in neg_multipliers:
        tag = f"negmult_{nm:.1f}"
        print(f"\n{'=' * 50}")
        print(f"  训练: neg_multiplier = {nm}")
        print(f"{'=' * 50}")

        models, calibrator = train_asym_weight_gbdt(ctx, extra_raw, neg_multiplier=nm)

        # 测试集评估
        pt = predict_asym_weight(ctx, models, calibrator, extra_raw, "test")
        result = evaluate_topk(pt, y_te, retf_te, ts_te)

        # 假阳性率评估 (全测试集)
        fp_info = eval_false_positive_rate(pt, y_te)

        # 保存预测
        pt_path = f"{config.DS_DIR}/BTC_gbdt_asym_{tag}_pt.npy"
        np.save(pt_path, pt)
        print(f"  [save] test 预测保存至: {pt_path}")

        pv = predict_asym_weight(ctx, models, calibrator, extra_raw, "meta_val")
        pv_path = f"{config.DS_DIR}/BTC_gbdt_asym_{tag}_pv.npy"
        np.save(pv_path, pv)
        print(f"  [save] meta_val 预测保存至: {pv_path}")

        all_results[nm] = result
        all_fp_rates[nm] = fp_info
        all_models[nm] = (models, calibrator)

        # 立即打印该配置结果
        print(f"\n  --- 结果: neg_multiplier={nm} ---")
        print(f"  top1% 准确率:   {result['accuracy']:.4f}")
        print(f"  平均收益:       {result['avg_ret_bps']:.1f} bps")
        print(f"  上涨收益:       {result['avg_ret_up_bps']:.1f} bps")
        print(f"  下跌收益:       {result['avg_ret_dn_bps']:.1f} bps")
        print(f"  交易次数:       {result['k']}")
        print(f"  交易/天:        {result['trades_per_day']:.1f}")
        print(f"  全量假阳性率:   {fp_info['fp_rate']:.4f} "
              f"({fp_info['fp_count']}/{fp_info['total_pred_up']})")
        print(f"  全量假阴性率:   {fp_info['fn_rate']:.4f} "
              f"({fp_info['fn_count']}/{fp_info['total_pred_dn']})")
        print(f"  全量准确率:     {fp_info['accuracy']:.4f}")
        print(f"  训练耗时:       {time.time() - t_start:.0f}s", flush=True)

        # 清理当前模型, 释放内存
        del models, calibrator, pt, pv
        gc.collect()

    # ================================================================
    # 4. 最终对比汇总
    # ================================================================
    print(f"\n{'=' * 65}")
    print(f"  非对称加权实验: 最终对比汇总")
    print(f"{'=' * 65}")

    print(f"\n{'=' * 65}")
    print(f"  top1% 准确率对比")
    print(f"{'=' * 65}")
    for nm in neg_multipliers:
        r = all_results[nm]
        marker = " <<< BASELINE" if nm == 1.0 else ""
        print(f"  neg_mult={nm:.1f}:  acc={r['accuracy']:.4f}  "
              f"收益={r['avg_ret_bps']:.1f} bps  "
              f"交易/天={r['trades_per_day']:.1f}{marker}")

    # 与 baseline 对比
    baseline = all_results[1.0]
    print(f"\n{'=' * 65}")
    print(f"  与 Baseline (neg_mult=1.0) 对比")
    print(f"{'=' * 65}")
    for nm in neg_multipliers:
        if nm == 1.0:
            continue
        r = all_results[nm]
        delta_acc = (r['accuracy'] - baseline['accuracy']) * 100
        delta_ret = r['avg_ret_bps'] - baseline['avg_ret_bps']
        print(f"  neg_mult={nm:.1f}:  "
              f"Δacc={delta_acc:+.2f}pp  "
              f"Δ收益={delta_ret:+.1f} bps  "
              f"acc={r['accuracy']:.4f}")

    print(f"\n{'=' * 65}")
    print(f"  假阳性率对比 (全测试集, 非仅 top1%)")
    print(f"{'=' * 65}")
    for nm in neg_multipliers:
        fp = all_fp_rates[nm]
        baseline_fp = all_fp_rates[1.0]
        delta_fp = (fp['fp_rate'] - baseline_fp['fp_rate']) * 100
        marker = " <<< BASELINE" if nm == 1.0 else ""
        print(f"  neg_mult={nm:.1f}:  "
              f"FP率={fp['fp_rate']:.4f} ({fp['fp_count']}/{fp['total_pred_up']})  "
              f"FN率={fp['fn_rate']:.4f} ({fp['fn_count']}/{fp['total_pred_dn']})  "
              f"全量准确率={fp['accuracy']:.4f}"
              f"{'  ΔFP=' + f'{delta_fp:+.2f}pp' if nm != 1.0 else ''}"
              f"{marker}")

    print(f"\n{'=' * 65}")
    print(f"  月度准确率明细 (neg_mult=1.0 baseline)")
    print(f"{'=' * 65}")
    r_base = all_results[1.0]
    for m in sorted(r_base['acc_by_month'].keys()):
        bar = "█" * int(r_base['acc_by_month'][m] * 40)
        print(f"  {m[-7:]}: acc={r_base['acc_by_month'][m]:.4f}  "
              f"n={r_base['n_by_month'][m]:4d}  {bar}")

    for nm in neg_multipliers:
        if nm == 1.0:
            continue
        r = all_results[nm]
        print(f"\n  --- 月度对比: neg_mult={nm:.1f} vs baseline ---")
        for m in sorted(r['acc_by_month'].keys()):
            if m in r_base['acc_by_month']:
                delta = (r['acc_by_month'][m] - r_base['acc_by_month'][m]) * 100
                sign = "+" if delta >= 0 else ""
                print(f"    {m[-7:]}: {r['acc_by_month'][m]:.4f} "
                      f"(Δ{sign}{delta:.2f}pp)  "
                      f"n={r['n_by_month'][m]:4d}")

    elapsed = time.time() - t_start
    print(f"\n{'=' * 65}")
    print(f"  总耗时: {elapsed:.0f}s", flush=True)
    print(f"{'=' * 65}")

    # 清理
    del ctx, extra_raw
    for nm in neg_multipliers:
        del all_models[nm]
    gc.collect()
    print(f"\n完成.", flush=True)


if __name__ == "__main__":
    main()